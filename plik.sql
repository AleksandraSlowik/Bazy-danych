...
CREATE TABLE schema_slowik.intersects AS
...
CREATE INDEX idx_intersects_rast_gist ON schema_slowik.intersects
USING gist (ST_ConvexHull(rast));

--dodanie ograniczeń constraints dla rastra

-- schema::name table_name::name raster_column::name
SELECT AddRasterConstraints('schema_slowik'::name, 'intersects'::name,'rast'::name);

--2 ST_Clip - obcięcie rastra na podstawie wektora
CREATE TABLE schema_slowik.clip AS
SELECT ST_Clip(a.rast, b.geom, true), b.municipality
FROM rasters.dem AS a, vectors.porto_parishes AS b
WHERE ST_Intersects(a.rast, b.geom) AND b.municipality like 'PORTO';

--3 ST_Union - połączenie
CREATE TABLE schema_slowik.union AS
...
CREATE TABLE schema_slowik.porto_parishes AS
WITH r AS (
...
DROP TABLE schema_slowik.porto_parishes; --> drop table porto_parishes first
CREATE TABLE schema_slowik.porto_parishes AS
WITH r AS (
SELECT rast FROM rasters.dem
LIMIT 1
...
create table schema_slowik.intersection as
...
CREATE TABLE schema_slowik.dumppolygons AS
...
CREATE TABLE schema_slowik.landsat_nir AS
SELECT rid, ST_Band(rast,4) AS rast
FROM rasters.landsat8;

--2 ST_Clip - przykład wycięcia rastra z innego rastra
CREATE TABLE schema_slowik.paranhos_dem AS
SELECT a.rid,ST_Clip(a.rast, b.geom,true) as rast
FROM rasters.dem AS a, vectors.porto_parishes AS b
WHERE b.parish ilike 'paranhos' and ST_Intersects(b.geom,a.rast);

--3 ST_Slope - generowanie nachylenia
CREATE TABLE schema_slowik.paranhos_slope AS
SELECT a.rid,ST_Slope(a.rast,1,'32BF','PERCENTAGE') as rast
FROM schema_slowik.paranhos_dem AS a;

--4 ST_Reclass - reklasyfikacja rastra
CREATE TABLE schema_slowik.paranhos_slope_reclass AS
SELECT a.rid,ST_Reclass(a.rast,1,']0-15]:1, (15-30]:2, (30-9999:3', '32BF',0)
FROM schema_slowik.paranhos_slope AS a;
...
--10 ST_TPI 
create table schema_slowik.tpi30 as --40.696s
select ST_TPI(a.rast,1) as rast
from rasters.dem a;

--dodanie indeksu przestrzennego
CREATE INDEX idx_tpi30_rast_gist ON schema_slowik.tpi30
USING gist (ST_ConvexHull(rast));

--dodanie ograniczeń constraints dla rastra
SELECT AddRasterConstraints('schema_slowik'::name, 'tpi30'::name,'rast'::name);

--10v2 ST_TPI - tylko dla gminy Porto 
create table schema_slowik.tpi30_porto as --1.828s
SELECT ST_TPI(a.rast,1) as rast
FROM rasters.dem AS a, vectors.porto_parishes AS b
WHERE ST_Intersects(a.rast, b.geom) AND b.municipality ilike 'porto';

--dodanie indeksu przestrzennego
CREATE INDEX idx_tpi30_porto_rast_gist ON schema_slowik.tpi30_porto
USING gist (ST_ConvexHull(rast));

--dodanie ograniczeń constraints dla rastra
SELECT AddRasterConstraints('schema_slowik'::name, 'tpi30_porto'::name,'rast'::name);
...
CREATE TABLE schema_slowik.porto_ndvi AS
WITH r AS (
SELECT a.rid,ST_Clip(a.rast, b.geom,true) AS rast
FROM rasters.landsat8 AS a, vectors.porto_parishes AS b
WHERE b.municipality ilike 'porto' and ST_Intersects(b.geom,a.rast)
...
--dodanie indeksu przestrzennego
CREATE INDEX idx_porto_ndvi_rast_gist ON schema_slowik.porto_ndvi
USING gist (ST_ConvexHull(rast));

--dodanie ograniczeń constraints dla rastra
SELECT AddRasterConstraints('schema_slowik'::name, 'porto_ndvi'::name,'rast'::name);

--2 Funkcja zwrotna - tworzenie funkcji
create or replace function schema_slowik.ndvi(
...
CREATE TABLE schema_slowik.porto_ndvi2 AS
WITH r AS (
SELECT a.rid,ST_Clip(a.rast, b.geom,true) AS rast
FROM rasters.landsat8 AS a, vectors.porto_parishes AS b
WHERE b.municipality ilike 'porto' and ST_Intersects(b.geom,a.rast)
...
r.rid,ST_MapAlgebra(
r.rast, ARRAY[1,4],
'schema_slowik.ndvi(double precision[], integer[],text[])'::regprocedure, --> This is the function!
'32BF'::text
) AS rast
FROM r;
...
--dodanie indeksu przestrzennego
CREATE INDEX idx_porto_ndvi2_rast_gist ON schema_slowik.porto_ndvi2
USING gist (ST_ConvexHull(rast));

--dodanie ograniczeń constraints dla rastra
SELECT AddRasterConstraints('schema_slowik'::name, 'porto_ndvi2'::name,'rast'::name);

--Eksport danych

--1 ST_AsTiff - utworzenie danych wyjściowych w reprezentacji binarnej pliku .tiff
SELECT ST_AsTiff(ST_Union(rast))
FROM schema_slowik.porto_ndvi;
...
/*SELECT lo_export(loid, 'C:\Users\DELL\Desktop\rasters\myraster.tiff') 
FROM tmp_out;
